- masquerade
  - 发音
    - 英 `/,mɑːskə'reɪd; ,mæs-/`
    - 美 `/,mæskə'red/`
  - 词义
    - v. 化装；伪装
    - `to pretend to be something or someone different`
    - n. 伪装；化妆舞会
    - `a formal dance or party where people wear  mask s  and unusual clothes`
  - 同根词
    - n.
      - `masking` 掩蔽；隐蔽
      - `masquer` 蒙面者；化装舞蹈者；戴假面具的人
      - `masquerader` 冒充者；戴假面具者；参加假面舞会的人
    - v.
      - `masking` 戴面具；掩饰；化装（mask的ing形式）
  - 近义词
    - vi. 化装；伪装
      - `make sb up`
      - `mask`
    - n. 伪装；化妆舞会
      - `dissemblance`
      - `costume ball`
    - vt. 化装；乔装
      - `make sb up`
  - 例句
    - `This caused issues in a DB2 V7 environment in that to go to another server, you had to "masquerade" as the original node and know their respective passwords.`
      - 这在 DB2 V7 环境中会导致一些问题，为了恢复到另一台服务器，您必须“伪装”成原先的节点，并且要知道它们各自的密码。
    - `What could a criminal do with a speech synthesis system that could masquerade as a human being?`
      - 想想看如果罪犯利用完美的语音合成系统伪装成别人会怎么样？

