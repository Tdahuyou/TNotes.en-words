- quarterly
  - 发音
    - 英 `/'kwɔːrtərli/`
    - 美 `/'kwɔrtɚli/`
  - 词义
    - adv. 按季度，一季一次
    - adj. 季度的，每季一次的
    - `produced or happening four times a year`
    - n. 季刊
    - `a magazine that is produced four times a year`
  - 记忆
    - 来自quarter(n. 季度)
  - 同根词
    - adj.
      - `quarterfinal` 四分之一决赛的
      - `quartering` 成直角的；四等分的
      - `quartic` 四次的
    - n.
      - `quarter` 四分之一；地区；季度；一刻钟；两角五分；节；vi. 住宿；驻扎；vt. 将…四等分；供某人住宿；num. 四分之一
      - `quartile` 四分位数；四分点
      - `quarterback` 橄榄球的四分卫
      - `quarterfinal` 四分之一决赛
      - `quartering` 四等分；四分法
      - `quartic` 四次式
      - `quartern` 四分之一品脱；四等分，四分之一
    - v.
      - `quartering` 把…四等分（quarter的ing形式）
    - vi.
      - `quarterback` （橄榄球中）担任四分卫
    - vt.
      - `quarterback` （以四分卫）指挥进攻；领导；操纵
  - 短语
    - `quarterly report` 季报，季度报告 
  - 例句
    - `It makes no difference whether dividends are paid quarterly or annually.`
      - 红利是按季度还是按年度支付没有区别。

