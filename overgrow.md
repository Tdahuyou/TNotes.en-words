- overgrow
  - 发音
    - 英 `/əʊvə'grəʊ/`
    - 美 `/'ovɚ,gro/`
  - 词义
    - v. 长满
    - `to grow over or across (an area, path, lawn, etc) `
  - 同根词
    - adj.
      - `overgrown` 蔓生的；生长过快的
    - n.
      - `overgrowth` 增生；繁茂；生长过度；过度发育
    - v.
      - `overgrown` 生长过度（overgrow的过去分词）
  - 例句
    - `The big banyan overgrow of my day of day of a block limb, hang below brawny and strong limb full each golden orange.`
      - 一棵遮天蔽日的大榕树长满了枝干，粗壮有力的枝干下面挂满了一个个金灿灿的橘子。
    - `These products have a chance to potentially overgrow their "M" and "P" roots, because they can be used to build a backbone of the enterprise-wide SOA.`
      - 这些产品有机会增长它们的“M”与“P”根，因为它们可以用于构建企业范围 SOA 的框架。
    - `Go on the ridge of field of overgrow fireweed , one pace is flabby accurate meeting falls.`
      - 走在长满杂草的田埂上，一步不稳准会跌倒。

