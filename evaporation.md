- evaporation
  ...
  - 词义
    - n. 蒸发；消失，不存在
  - 记忆
    - 来自evaporate(v. 蒸发； 消失)； e(出) + vapor(水汽) + ate(使…) → 使水汽出去 → 蒸发
  - 同根词
    - adj.
      - `evaporated` 浓缩的；脱水的；蒸发干燥的
      - `evaporative` 蒸发的，成为蒸气的
      - `evaporable` 易蒸发性的；易挥发的
    - n.
      - `evaporite` [岩] 蒸发岩，蒸发盐
      - `evaporometer` 蒸发计
    - v.
      - `evaporated` 蒸发；消失（evaporate的过去式）；失去水分
    - vi.
      - `evaporate` 蒸发，挥发；消失，失踪
    - vt.
      - `evaporate` 使……蒸发；使……脱水；使……消失
  - 近义词
    - n. [物][化学]蒸发；消失
      - `extinction`
      - `vaporization`
