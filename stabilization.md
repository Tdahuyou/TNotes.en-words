- stabilization
  - 发音
    - 英 `/,steɪbɪlaɪ'zeɪʃən/`
    - 美 `/,stebɪlɪ'zeʃən/`
  - 词义
    - n. 稳定性
  - 同根词
    - adj.
      - `stable` 稳定的；牢固的；坚定的
      - `stabilized` 稳定的；减摇的
      - `stabile` 稳定的；固着的
    - n.
      - `stable` 马厩；牛棚
      - `stability` 稳定性；坚定，恒心
      - `stabilizer` [助剂] 稳定剂；稳定器；安定装置
      - `stabilizing` 稳定化；稳定化处理
      - `stabile` 固定的雕刻物
      - `stableness` 稳定性
    - v.
      - `stabilized` 稳定（stabilize的过去分词）
      - `stabilizing` 使安定（stabilize的ing形式）
    - vi.
      - `stable` 被关在马厩
      - `stabilize` 稳定，安定
      - `stabilise` 稳定；安定
    - vt.
      - `stable` 赶入马房
      - `stabilize` 使稳固，使安定
      - `stabilise` 使…坚固；使…安定；装稳定器（等于stablize）
  - 近义词
    - n. 稳定；稳定化
      - `steadiness`
  - 短语
    - `stabilization pond` 稳定塘；氧化池，稳定池 
    - `stabilization fund` 平准基金；稳定基金 
    - `soil stabilization` 土壤稳定；土壤加固法 
    - `thermal stabilization` 热安定性化；热加固 
  - 例句
    - `On this point, RUP offers extensive guidance by showing you the prioritization technique of use cases and architecture stabilization.`
      - 在这一点上，RUP 通过展示用例和架构稳定性的优先化技术来提供广泛的指导。
    - `Our goal must be the stabilization and normalization of life in Gaza.`
      - 我们的目标必须是使加沙人民能过上稳定、正常的生活。

