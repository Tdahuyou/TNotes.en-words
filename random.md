- random
  ...
  - 同根词
    - adj.
      - `randomized` [数] 随机化的，随机的
    - adv.
      - `randomly` 随便地，任意地；无目的，胡乱地；未加计划地
    - n.
      - `randomness` 随意；无安排；不可测性
      - `randomization` [数] 随机化，不规则分布；随机选择
    - v.
      - `randomized` 使随机化；做任意排列（randomize的过去分词）
    - vt.
      - `randomize` 使随机化；（使）作任意排列或不规则分布
  - 近义词
    - adj. [数]随机的；任意的；[建]胡乱的
      - `stochastic`
      - `arbitrary`
