- measure
  ...
  - 同根词
    - adj.
      - `measured` 量过的；慎重的；基于标准的；有规则的
      - `measurable` 可测量的；重要的；重大的
      - `measuring` 测量用的
      - `measureless` 无限的，不可量的
    - adv.
      - `measurably` 可以测定的程度；可视地
    - n.
      - `measurement` 测量；[计量] 度量；尺寸；量度制
      - `measuring` 测量；衡量
      - `measurability` 可测量性
      - `measurer` [测] 测量器；[测] 测量员
    - v.
      - `measured` 测量，判断（measure的过去分词）
      - `measuring` 测量（measure的ing形式）
  - 近义词
    - n. 测量；措施；程度；尺寸
      - `extent`
      - `degree`
