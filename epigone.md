- epigone
  - 发音
    - 英
      - `/'epɪgəʊn/`
    - 美
      - `/undefined/`
  - 词义
    - n.
      - 追随者,模仿者
        - `an inferior follower or imitator `
  - 近义词
    - n.
      - 追随者；天赋不如先辈的子孙；蹩脚的模仿者（等于epigon）
        - `follower`
        - `old faithful`
  - 例句
    - `We think, IT tycoon must hour is vigilant all sorts of competitors and epigone , avoid in-house error at the same time.`
      - 我们认为，IT巨头必须时刻警惕各种竞争对手和追随者，同时避免内部失误。
    - `You are vogue and beautiful epigone, yearn for the perfect outline of cap-a-pie , blossom a glorious of unapproachable evil spirit be puzzled.`
      - 你是时尚和漂亮的追随者，渴望从头到脚的完美轮廓，绽放出无与伦比的魅惑光彩。

